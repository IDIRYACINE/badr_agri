...
class Trees extends Table {
  ...
  TextColumn get subType => text().references(TreeSubTypes, #id)(); // Foreign key
...
class Gardens extends Table {
  ...
  TextColumn get coordinatesId => text().references(GpsCoordinates, #id)(); // Foreign key
...
class GardenSections extends Table {
  ...
  TextColumn get treeSubType => text().references(TreeSubTypes, #id)(); // Foreign key
...
class SectionLines extends Table {
  TextColumn get id => text()(); // Primary key
  TextColumn get gardenSectionId => text().references(GardenSections, #id)(); // Foreign key
...
class TreeHistories extends Table {
  TextColumn get id => text()(); // Primary key
  TextColumn get treeId => text().references(Trees, #id)(); // Foreign key to Trees table
  TextColumn get treeHistoryOptionId => text().references(TreeHistoryOptions, #id)(); // Foreign key to TreeHistoryOptions table
...
class TreeHistoryOptions extends Table {
  TextColumn get id => text()(); // Primary key
  TextColumn get name => text().withLength(min: 1, max: 50)(); // Option name (e.g., pruning, watering, etc.)
